...
import { createTech, deleteTech, updateTech } from '@/actions/tech.action';
...
export default function TechPage() {
    const { techs } = useAdminStore();
    const techFields: TypeFields = [
        { name: 'name', label: 'Tecnología', type: 'text' as const, required: true, placeholder: 'Ej: Python, TypeScript, Java...' },
        { name: 'mastery_level', label: 'Nivel de Expertis', type: 'text' as const, required: true, placeholder: 'Ej: Principiante, Intermedio, Avanzado' },
        { name: 'category', label: 'Categoría', type: 'text' as const, required: true, placeholder: 'Ej: FrontEnd, BackEnd, BI, UX/UI, Data Analist...' },
    ];
    ...
    const handleSubmite = async (formDataObj: Record<string, unknown>): Promise<ActionResponse> => {
        ...
        if (operation === 'create') {
            return await createTech({ success: false, msg: '', errors: [] }, formData);
        } else if (operation === 'edit' && data?.id) {
            return await updateTech(data.id, { success: false, msg: '', errors: [] }, formData);
        }
    ...
    return (
        ...
        >
                    ...
                    {techs && techs?.length > 0
                        ? <div className='py-6 align-middle sm:px-6 lg:px-8 bg-gray-900 text-white rounded-3xl space-y-6 hidden md:block'>
                            <div className='space-y-6'>
                                <p>
                                    Aquí se administran las tecnologías que se mostrarán en tu portafolio
                                </p>
                        ...
                        : <div className="text-center py-12 align-middle sm:px-6 lg:px-8 bg-gray-900 text-white rounded-3xl w-2xl">
                            <h3 className="text-lg font-medium mb-2 pb-4">No se han encontrado tecnologías</h3>
                            <p className="text-muted-foreground mb-2">
                                Aún no hay tecnologías en tu portafolio.
                            </p>
                        </div>
                    }
                </div>
            ...
            <CrudDialog
                ...
                description={
                    operation === 'create'
                        ? 'Completa los datos para crear una nueva tecnología'
                        : 'Modifica los datos de la tecnología'
                }
                data={data}
                onSubmit={handleSubmite}
                onDelete={handleDelete}
                deleteConfirmationDescription={`Esta acción no se puede deshacer. Se eliminará la tecnología "${data?.name}" permanentemente.`}
            >
